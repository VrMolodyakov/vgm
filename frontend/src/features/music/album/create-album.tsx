...
const CreateAlbumForm: React.FC = () => {
  ...
  const [tracklist, setTracklist] = useState<Track[]>([{ title: "", duration: "" }])
  const [credits, setCredits] = useState<Credit[]>([{ profession: "", person_id: -1 }])
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="album-from">
      <h1>Create Album</h1>
      <label>Title</label>
